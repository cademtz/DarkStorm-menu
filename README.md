...
Just a neat menu source for the TF2 DarkStorm base
 ...
 addFloat(int index, char name[30], float *value, int min, int max, int step) // Step is how much the item will increment / decrement
 ```
 ...
 Then, to actually use those values for the menu to modify, insert them in the createList void in Menu.cpp like so:
 ```cpp
 ...
 void Menu::createList()
 {
  ...
  if (gEsp.espTab) // Make sure you use an if statement with your tab variable, otherwise it will appear regardless of the tab
  {
